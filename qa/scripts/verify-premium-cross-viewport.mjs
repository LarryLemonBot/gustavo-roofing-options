...
const expression = `(() => {
  ...
  return {
    ...
    textUsCount: Array.from(document.querySelectorAll('a')).filter(a => /Text Us Your Photos|Text Photos/i.test(a.textContent || '')).length,
    hasTrustCopy: bodyText.includes('Family-owned and operated') && bodyText.includes('Owner-led final review'),
    forbiddenClaimHits: ['guaranteed insurance approval','best roofer','BBB','GAF certified','FORTIFIED certified','license number','insurance carrier','policy number'].filter(claim => bodyText.toLowerCase().includes(claim.toLowerCase())),
...
const pages = [['home','/'], ['services','/services'], ['photos','/photos'], ['areas','/areas'], ['process','/process'], ['contact','/contact']];
const viewports = [
...
try {
  ...
  const issues = captures.flatMap(c => {
    ...
    if (r.textUsCount < 1) arr.push('missingTextUsCTA');
